...
src_compile() {
	...
	case ${CHOST} in
	*-darwin*)
		my_ldflags="-dynamiclib -install_name ${EPREFIX}/usr/$(get_libdir)/"
		;;
	*)
		my_ldflags="-shared -Wl,-soname,"
		;;
	esac
	echo "$(tc-getCC) ${LDFLAGS} ${my_ldflags}libufconfig$(get_libname ${PV}) -o libufconfig$(get_libname ${PV}) UFconfig.lo"
	$(tc-getCC) ${LDFLAGS} ${my_ldflags}libufconfig$(get_libname ${PV}) -o libufconfig$(get_libname ${PV}) UFconfig.lo || die
...
src_install() {
	dolib.so libufconfig$(get_libname ${PV}) || die
